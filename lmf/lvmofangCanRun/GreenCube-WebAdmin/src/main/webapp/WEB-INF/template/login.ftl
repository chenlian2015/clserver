...
[@ui.page bodyClass="login"]
	...
	<div class="logo">
		<h2 style="font-size: 24px;color:white;">“绿魔方”后台管理平台</h2>
	</div>
		...
		<form class="form-vertical login-form" id="login-form">
			...
			<div class="control-group">
				...
				<div class="controls">
					<div class="input-icon left">
						<i class="icon-user"></i>
						<input class="m-wrap placeholder-no-fix" type="text" placeholder="账号" class="bar validate[required]" maxlength="20" name="username" value="admin"/>
					</div>
				</div>
			</div>
			...
			<div class="control-group">
				...
				<div class="controls">
					<div class="input-icon left">
						<i class="icon-lock"></i>
						<input class="m-wrap placeholder-no-fix" type="password" placeholder="密码" class="bar validate[required]" maxlength="20"  name="password" value="12345678"/>
					</div>
				</div>
			</div>
        ...
        doLogin:function (){
        	Submit.submitForm("/login/dologin", "login-form",function(result){
				if(result.success == false){
					...
					$.cookie("_greencube_user_", TJsonUtils.json2String(result.data.u), {
						expires : expiresDate,
						path : '/'
					});
