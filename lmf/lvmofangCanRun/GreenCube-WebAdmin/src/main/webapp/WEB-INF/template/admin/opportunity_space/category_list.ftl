...
<script>
	...
	$(".btnsubmit").click(function(){
		...
		Submit.submitForm('/admin/opportunity_space/category_update' , 'addform', function(result){
	...
	function removeNode(id,classCode,ts){
	  if(id=="")
	  {
	   $(ts).parent().parent().remove();
	  	...
	  	Submit.invok('/admin/opportunity_space/category_delete?id=' + id+'&classCode='+classCode, function(result){
		if(result.success==true)
		{
		// 删除掉格子
			$(ts).parent().parent().remove();
	...
	function moveLeft (ts, nodeid) {
		Submit.invok('/admin/opportunity_space/setting/moveLeft?id=' + nodeid, function(){
