				...
				<div class="span12">
					<div class="portlet box purple">
						<div class="portlet-title">
							<div class="caption">
								<i class="icon-comments"></i>栏目管理
							</div>
							<div class="actions">
								<a href="javascript:void(0);" class="btn blue btnAddRoot"><i class="icon-plus"></i>&nbsp;添加栏目</a>
							</div>
						</div>
...
<script>
	...
	$(".btnsubmit").click(function(){
		...
		Submit.submitForm('/admin/cms/channel_update' , 'addform', function(result){
	...
	function makeJson(){
		...
		if (uls.length > 0){
			$.each(uls, function(index,elUl){
				cval = $(elUl).children("li").children("input[data-role='name']").attr("value");
	...
	function getJsonValue(parentNode, elUl) {
		var cval = elUl.children("li").children("input[data-role='name']").attr("value");
		...
		childNode["sub"] = new Array();
		
		parentNode["sub"].push(childNode);
	...
	alert(classCode);
	  if(id=="")
	  {
	   $(ts).parent().parent().remove();
	  	...
	  	Submit.invok('/admin/cms/channel_delete?id=' + id+'&classCode='+classCode, function(result){
		if(result.success==true)
		{
		// 删除掉格子
			$(ts).parent().parent().remove();
	...
	function moveLeft (ts, nodeid) {
		Submit.invok('/admin/cms/moveLeft?id=' + nodeid, function(){
