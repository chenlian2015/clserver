...
public class ProductDaoImpl extends RedisDaoSupportImpl<Product, String> implements ProductDao {

	@Override
	public List<Product> getHotProducts(int count) {
		String sql = "select g.C_ID from T_PRODUCT where N_STATUS_SHELF = 1 and N_STATUS_AUDIT = 1 and N_STATUS_DELETE = 0 and t.N_IS_RECOMMEND = 1 limit 0, "
				+ count;
		...
		try {
			...
			List<Product> goods = (List<Product>) jedisTemplate.execute(new JedisAction() {
				@Override
				public Object action(Jedis jedis) {
					...
					for (Map idMap : ids) {
						...
						String goodsXml = jedis.hget(RedisDaoSupportImpl.POOL + "ProductHot", id);
						if (goodsXml != null) {
							Product g = (Product) x.fromXML(goodsXml);
							if (g != null) {
								goods.add(g);
							}
						} else {
							...
							if (g != null) {
								...
								jedis.hset(RedisDaoSupportImpl.POOL + "ProductHot", id, xml);
							}
							goods.add(g);
						}
					}
