...
import com.cnd.greencube.web.base.util.FileUtils;
import com.cnd.greencube.web.base.util.JsonUtils;
import com.cnd.greencube.web.base.util.StringUtils;
import com.cnd.greencube.web.base.util.cutimage.CutImage;
import com.cnd.greencube.web.base.util.cutimage.ImageCut;
import com.cnd.greencube.web.base.util.myutil.Base64;
...
public class FileUploadController extends BaseController {
	...
	String uploadphoto() {
		try {
			...
			if (logo != null && logo.getSize() > 0) {
				// 处理保存文件
				file = UploadFileManager.uploadFile(logo);
			}
	...
	String forkindeditor() {
		...
		if (!StringUtils.isEmpty(name)) {
			...
			try {
				...
				if (uploadfile != null && uploadfile.getSize() > 0) {
					// 处理保存文件
					file = UploadFileManager.uploadFile(uploadfile);
	...
	String data() throws Exception {
		...
		while (matcher.find()) { // 找出base64图片元素
			...
			String src = StringUtils.substringBetween(str, "src=\"", "\"");
			...
			if (!StringUtils.isEmpty(base64ImgData)) {
				...
				if (file != null) {
					// 保存文件
					TFile upfile = UploadFileManager.uploadFile(file);
					UploadFileData u = new UploadFileData();
