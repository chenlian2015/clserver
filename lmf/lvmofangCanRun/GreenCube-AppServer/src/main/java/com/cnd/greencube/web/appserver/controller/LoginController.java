...
public class LoginController extends BaseController {
	...
	public String login(final String username, final String password, final String verifycode, final boolean pwdEncrypted, HttpSession session, final String usertype) throws Exception {
		try {
			String result = thriftTemplate.execute("LoginService", new ThriftAction() {
				...
				public String action(TServiceClient paymentService) throws Exception {
					return ((LoginService.Client) paymentService).login(username, password, verifycode, pwdEncrypted, usertype);
				}
			});
