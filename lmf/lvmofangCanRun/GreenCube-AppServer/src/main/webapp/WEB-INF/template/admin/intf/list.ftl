									...
									[#list services as s]
										[#list s.interfaces as i]
											<tr class="odd gradeX">
												...
												<td nowrap>
													<a href="/admin/intf/detail?id=${i.id }" class="btn green mini"><i class="icon-edit"></i> 明细</a>
												</td>
											</tr>
										[/#list]
									[/#list]
	...
	$(document).ready(function(){
		...
		$(".reloadInters").on("click",function(){
			if (window.confirm("确定要刷新接口列表吗？")){
				Submit.invok("/admin/intf/reload",function(){
					window.location.reload(true);
				});				
			}
		});
		$("#selectedService").change(function(){
			window.location.assign("/admin/intf/list?service=" + $(this).val());
		});
	});
